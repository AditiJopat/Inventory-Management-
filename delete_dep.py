...
def delete_supplier(sid):
    try:
        cursor.execute("DELETE FROM department WHERE depid=?", (sid,))
        conn.commit()
        print("<p>Department details deleted successfully.</p>")
    except Exception as e:
        print("<p>Error deleting Department details: {}</p>".format(str(e)))

# Form to input supplier ID for deletion
print("<div style='text-align: center; padding: 20px;'>")
print("<form method='post' action=''>")
print("<label for='sid' style='color: #800000; font-weight: bold;'>Enter Department ID to Delete:</label>")
