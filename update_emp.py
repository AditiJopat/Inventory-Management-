...
def update_employee(empid, field, new_value):
    try:
        cursor.execute("UPDATE employee SET {}=? WHERE empid=?".format(field), (new_value, empid))
        conn.commit()
        print("<p>Employee details updated successfully.</p>")
    except Exception as e:
        print("<p>Error updating employee details: {}</p>".format(str(e)))

# Form to input employee details for update
print("<div style='text-align: center; padding: 20px;'>")
print("<form method='post' action=''>")
print("<label for='empid' style='color: #800000; font-weight: bold;'>Enter Employee ID:</label>")
print("<input type='text' id='empid' name='empid' style='margin-left: 10px; padding: 5px; border: 1px solid #800000;'>")
print("<br>")
print("<label for='field' style='color: #800000; font-weight: bold;'>Enter Field to Update:</label>")
print("<select id='field' name='field' style='margin-left: 10px; padding: 5px; border: 1px solid #800000;'>")
print("<option value='fname'>First Name</option>")
print("<option value='mname'>Middle Name</option>")
print("<option value='lname'>Last Name</option>")
print("<option value='desiationid'>Designation ID</option>")
print("<option value='qualification'>Qualification</option>")
print("<option value='joiningdate'>Joining Date</option>")
print("<option value='phoneno'>Phone Number</option>")
print("<option value='dateofleaving'>Date of Leaving</option>")
...
print("</select>")
print("<br>")
print("<label for='new_value' style='color: #800000; font-weight: bold;'>Enter New Value:</label>")
print("<input type='text' id='new_value' name='new_value' style='margin-left: 10px; padding: 5px; border: 1px solid #800000;'>")
print("<br>")
...
if "action" in form and form["action"].value == "Update":
    if "empid" in form and "field" in form and "new_value" in form:
        empid = form["empid"].value
        field = form["field"].value
        new_value = form["new_value"].value
        update_employee(empid, field, new_value)
    else:
