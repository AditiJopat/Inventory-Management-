...
if "action" in form and form["action"].value == "OK":
    if "itemname" in form:
        ...
        if item_id:
            ...
            cursor.execute("SELECT SUM(issuequantity) FROM ivtable WHERE itemid=?", (item_id[0],))
            total_issue_quantity = cursor.fetchone()[0]  # Fetch the sum of issue quantity
            # Fetch quantity bought from itemdetails table for the item ID
            cursor.execute("SELECT quantitybought FROM rvtable WHERE itemid=?", (item_id[0],))
            quantity_bought = cursor.fetchone()[0]  # Fetch quantity bought
            ...
            if quantity_available >= 0:
                ...
                print(f"<p>Quantity Required: {sumreq[0]}</p>")
                print("</div>")
