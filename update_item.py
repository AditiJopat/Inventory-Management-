...
def update_employee(supid, field, new_value):
    try:
        cursor.execute("UPDATE itemtable SET {}=? WHERE itemid=?".format(field), (new_value, supid))
        conn.commit()
...
print("<option value='itemname'>itemname</option>")
print("<option value='supplierid'>supplierid</option>")
