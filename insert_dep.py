...
def insert_employee(depname, depid):
    try:
        cursor.execute("INSERT INTO department (depname, depid) VALUES ( ?, ?)",
                       ( depname, depid))
        conn.commit()
        print("<p>Department details inserted successfully.</p>")
    except Exception as e:
        print("<p>Error inserting Department details: {}</p>".format(str(e)))
...
print("<label for='depname' style='color: #800000; font-weight: bold;'>Department Name:</label>")
print("<input type='text' id='depname' name='depname' style='margin-left: 10px; padding: 5px; border: 1px solid #800000;'><br>")
...
if "action" in form and form["action"].value == "Insert":
    if all(key in form for key in ['depname', 'depid']):
        depname = form["depname"].value
        depid = form["depid"].value
      
        insert_employee(depname, depid)
    else:
