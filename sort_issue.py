...
# Add some spacing between navbar and "login Details" heading
...
<div style='text-align: center; padding: 20px;'>
    <form method='post' action=''>
        ...
        <select id="sort" name="sort" style="margin-left: 10px; padding: 5px; border: 1px solid #800000;">
            ...
            <option value="ivnumber">ivnumber</option>
            <option value="quantity">quantity</option>
            <option value="rvnumber">rvnumber</option>
...
if "action" in form:
    ...
    if action == "Display":
        cursor.execute("SELECT * FROM issue voucher")
        ...
        cursor.execute(f"SELECT * FROM issuevoucher ORDER BY {sort_query}")
        employees = cursor.fetchall()
        for employee in employees:
            print("<tr>")
            for field in employee:
    ...
    print("<tr><td colspan='14'>No login found.</td></tr>")
