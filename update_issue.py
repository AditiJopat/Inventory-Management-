...
def update_employee(supid, field, new_value):
    try:
        cursor.execute("UPDATE issuevoucher SET {}=? WHERE itemid=?".format(field), (new_value, supid))
        conn.commit()
        print("<p>itemtable details updated successfully.</p>")
...
print("<label for='supid' style='color: #800000; font-weight: bold;'>Enter item ID:</label>")
...
print("<option value='depid'>depid</option>")
print("<option value='itemid'>itemid</option>")
print("<option value='ivnumber'>ivnumber</option>")
print("<option value='quantity'>quantity</option>")
print("<option value='rvnumber'>rvnumber</option>")
print("<option value='dateofleaving'>dateofleaving</option>")
