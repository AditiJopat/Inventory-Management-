...
def update_employee(supid, field, new_value):
    try:
        cursor.execute("UPDATE rvtable SET {}=? WHERE rvnumber=?".format(field), (new_value, supid))
        conn.commit()
        print("<p>rvtable details updated successfully.</p>")
    except Exception as e:
        print("<p>Error updating itemtable details: {}</p>".format(str(e)))

# Form to input employee details for update
print("<div style='text-align: center; padding: 20px;'>")
print("<form method='post' action=''>")
print("<label for='supid' style='color: #800000; font-weight: bold;'>Enter rvnumber :</label>")
print("<input type='text' id='supid' name='supid' style='margin-left: 10px; padding: 5px; border: 1px solid #800000;'>")
print("<br>")
...
print("<option value='rvnumber'>rvnumber</option>")
print("<option value='itemid'>itemid</option>")
print("<option value='amount'>amount</option>")
print("<option value='quantity'>quantity</option>")
print("<option value='paymentdate'>paymentdate</option>")
print("<option value='modeofpayment'>modeofpayment</option>")
print("<option value='receivedbyempid'>receivedbyempid</option>")
