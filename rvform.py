...
# Payment Date Input
print("<label for='paymentDate' style='color: red;'>Payment Date:</label>")
...
if form.getvalue("itemname"):
    ...
    cursor.execute("SELECT rvnumber FROM rvprint ORDER BY rvnumber DESC LIMIT 1")
    ...
    cursor.execute("INSERT INTO rvprint (rvnumber,itemid,amount, paymentdate, modeofpayment,quantity) VALUES (?,?, ?, ?, ?, ?)",
                       (new_rv_number,itemname, amount, payment_date, mode_of_payment,quantity))
    conn.commit()
    if itemname:
