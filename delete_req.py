...
def delete_supplier(sid):
    try:
        cursor.execute("DELETE FROM requirement WHERE itemid=?", (sid,))
        conn.commit()
        print("<p>requirement details deleted successfully.</p>")
    except Exception as e:
        print("<p>Error deleting requirement details: {}</p>".format(str(e)))

# Form to input supplier ID for deletion
print("<div style='text-align: center; padding: 20px;'>")
print("<form method='post' action=''>")
print("<label for='sid' style='color: #800000; font-weight: bold;'>Enter itemid to Delete:</label>")
